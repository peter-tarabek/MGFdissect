...
# this script apart from combining the MFGs searches also a custom .db database
# (SQLite, the search table must be named "ions", example included in the repo)
# and assigns a numeric value based on previous analysis relating to the presence
# of compounds in blanks (0_ identified previously in samples but not present in blank,
# 1_ identified previously and also present in blank,
# NA_ present in the database but not identified previously)
...
# select polarity in a list dialog
modes <- list("POS", "NEG")

polarity <- dlg_list(choices = modes, preselect = modes[[1]], title = "Select polarity")$res

if (identical(polarity, character(0))) {
...
# connect to database
mydb <- dbConnect(RSQLite::SQLite(), selected_db_file) 
ionsTab <- dbGetQuery(mydb, 'SELECT ID, "m/z", "RT/min", ESIpolarity, Identity, blank FROM ions')
custom_db <- as.data.table(ionsTab)[ESIpolarity == polarity]
dbDisconnect(mydb)
...
el <- list()
for (i in 1:length(s)) { # this is actually surprisingly very fast...
...
while (nrow(dt) > 1) {
  ...
  if (nrow(rt_win > 0)) {
    ...
    if (nrow(mz_win > 0)) {
      ...
      ident <- paste(arr_mut_mz_win$blank[1], arr_mut_mz_win$Identity[1], sep = "_")
    } else {
      ident <- ""
    }
  } else {
...
file.rename("output.txt", paste0(pasted, "merged_E_", polarity, ".mgf"))
